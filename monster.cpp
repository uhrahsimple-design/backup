...
void Monster::dropLoot(Container* corpse, Creature* killer)
{
	if (!corpse || !lootDrop) {
		return;
	}
	...
	// Pouch Bag: only coins generated by monster loot are collected automatically.
	Player* player = nullptr;
	Creature* owner = killer;
	while (owner) {
		if (Player* ownerPlayer = owner->getPlayer()) {
			player = ownerPlayer;
			break;
		}
	...
	Item* pouch = player->getPouchBag();
	Container* pouchContainer = pouch ? pouch->getContainer() : nullptr;
	if (!pouchContainer) {
		return;
	}
	...
	while (true) {
		...
		if (g_game.internalMoveItem(coin->getParent(), pouchContainer, INDEX_WHEREEVER,
		                            coin, coin->getItemCount(), nullptr, 0, player) != RETURNVALUE_NOERROR) {
			break;
		}
	}
}
