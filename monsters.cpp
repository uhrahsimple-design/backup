...
constexpr uint32_t LOOT_BOOST_STORAGE = 32424;
constexpr uint32_t LOOT_BOOST_PERCENT = 50;
	...
	if (!owner || owner->getStaminaMinutes() > 840) {
		...
		for (auto it = lootItems.rbegin(), end = lootItems.rend(); it != end; ++it) {
			...
			for (Item* item : itemList) {
				...
				// Ativa o efeito somente para item NÃO acumulável
				if (!Item::items[item->getID()].stackable) {
					lootEffect = true;
				}
			}
		}

		// Efeito do Loot Boost
		int32_t lootStorage = 0;

		if (lootEffect &&
			owner &&
			owner->getStorageValue(32424, lootStorage) &&
			lootStorage > time(nullptr)) {

			g_game.addMagicEffect(
				corpse->getPosition(),
				CONST_ME_LOOT
			);
		}
...
std::vector<Item*> MonsterType::createLootItem(const LootBlock& lootBlock, Player* player)
{
	int32_t itemCount = 0;
	...
	if (player && player->getStorageValue(32424, storageValue) && storageValue > time(nullptr)) {

		// +50% na chance de loot
		chance = std::min<uint32_t>(
			MAX_LOOTCHANCE,
			static_cast<uint32_t>(
				(static_cast<uint64_t>(chance) * 1500) / 100
				)
		);
...
bool MonsterType::createLootContainer(Container* parent, const LootBlock& lootblock, Player* player)
{
	...
	for (; it != end && parent->size() < parent->capacity(); ++it) {
		auto itemList = createLootItem(*it, player);
		for (Item* tmpItem : itemList) {
			if (Container* container = tmpItem->getContainer()) {
				if (!createLootContainer(container, *it, player)) {
					delete container;
				} else {
