...
void Combat::CombatHealthFunc(Creature* caster, Creature* target, const CombatParams& params, CombatDamage* data)
{
	...
	// - Critical Hit: doubles the damage when the attacker's accumulated chance succeeds.
	// - Dodge: completely avoids the hit when the target's accumulated chance succeeds.
	if (caster && target && caster != target && damage.primary.value < 0) {
		if (Player* attackerPlayer = caster->getPlayer()) {
			const int32_t criticalChance = std::min<int32_t>(100, std::max<int32_t>(0, attackerPlayer->getCriticalHitChance()));
			if (criticalChance > 0 && uniform_random(1, 100) <= criticalChance) {
				damage.primary.value *= 2;
				damage.secondary.value *= 2;
				g_game.addMagicEffect(target->getPosition(), 37);
			}
		}

		if (Player* targetPlayer = target->getPlayer()) {
			const int32_t dodgeChance = std::min<int32_t>(100, std::max<int32_t>(0, targetPlayer->getDodgeChance()));
			if (dodgeChance > 0 && uniform_random(1, 100) <= dodgeChance) {
				damage.primary.value = 0;
				damage.secondary.value = 0;
				g_game.addMagicEffect(target->getPosition(), 38);
				return;
			}
		}
	}
